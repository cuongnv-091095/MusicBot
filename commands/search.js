...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (!name) return interaction.reply({ content: `:x:  Please enter a valid song name.`, ephemeral: true }).catch(e => { })

    const res = await client.player.search(name, {
      requestedBy: interaction.member,
      searchEngine: QueryType.AUTO
    });
    if (!res || !res.tracks.length) return interaction.reply({ content: `:x:  No search results found.`, ephemeral: true }).catch(e => { })
    ...
    embed.setTimestamp();
    embed.setFooter({ text: `Customize by CươngNV from DIYEverything.xyz with ❤️` })
    ...
    collector.on('collect', async (query) => {
      ...
      if (!value || value <= 0 || value > maxTracks.length) return interaction.reply({ content: `:x:  Error: select a song **1** to **${maxTracks.length}** and write send or type **cancel** and cancel selection.`, ephemeral: true }).catch(e => { })
      ...
      try {
        ...
        return interaction.reply({ content: `:x:  I can't join audio channel.`, ephemeral: true }).catch(e => { })
      }
